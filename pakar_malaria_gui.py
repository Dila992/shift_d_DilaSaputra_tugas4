...
prolog = Prolog()
prolog.consult("pakar_malaria_gui.pl")  # pastikan file .pl berada di folder yang sama

# Ambil semua gejala yang ada di file Prolog
gejala_list = []
...
def konsultasi_prolog():
    ...
    # Reset fakta dinamis di Prolog
    list(prolog.query("retractall(gejala_pos(_))"))
    list(prolog.query("retractall(gejala_neg(_))"))

    # Tambahkan fakta gejala_pos ke Prolog
    for g in gejala_pos:
        prolog.assertz(f"gejala_pos({g})")

    # Inferensi penyakit berdasarkan gejala_pos
    kemungkinan = set()
    for p in prolog.query("penyakit(P), gejala(G, P), gejala_pos(G)"):
        kemungkinan.add(p["P"])
    ...
    if kemungkinan:
        ...
        kotak_pertanyaan.insert(END, "Tidak ditemukan kemungkinan penyakit.\n")
...
# GUI setup
root = Tk()
root.title("Sistem Pakar Diagnosis Malaria")
